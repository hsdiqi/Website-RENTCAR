...
function Mobil() {
  ...
  const fetchCarsAndCategories = () => {
    axios
      .get("http://localhost:3003/api/admin/mobil")
      .then((response) => {
        ...
        if (Array.isArray(resData.category)) {
          setCategory(["Semua", ...resData.category])
        }
      })
      .catch((error) => {
  ...
  const handlingHapus = (id) => {
    if (window.confirm("Apakah Anda yakin ingin menghapus kendaraan ini?")) {
      axios
        .put("http://localhost:3001/api/admin/mobil/del/", { id })
        .then((response) => {
  ...
  const addHandling = (event) => {
    ...
    axios
      .post("http://localhost:3003/api/cars/addCar", formData, {
        headers: {
  ...
  const handleSimpan = () => {
    ...
    formData.append("service", selectedCar.TERAKHIR_SERVICE)
    ...
    axios
      .put(`http://localhost:3003/api/admin/mobil/update/`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      })
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-gray-700 mt-2 font-medium">Kelola data kendaraan rental</p>
            </div>
          </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {cars.map((car, index) => (
                        <tr key={car.ID_KENDARAAN} className="hover:bg-gray-50 transition duration-150">
                          ...
                          <td className="px-6 py-4">
                            <div>
                              ...
                              <div className="text-sm text-gray-600 font-medium">
                                ...
                                {car.NOPOL} • <i className="fas fa-palette mr-1"></i>
                                {car.WARNA}
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-4 text-sm font-bold text-gray-900">{car.TIPE_KENDARAAN}</td>
                          <td className="px-6 py-4">
                            <span
                          ...
                          <td className="px-6 py-4 text-sm font-bold text-gray-900">{formatCurrency(car.HARGA)}</td>
                          <td className="px-6 py-4">
                  ...
                  {isEditing ? (
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-blue-600 font-medium"
                            value={anyBook}
                            onChange={(e) => setAnyBook(Number.parseInt(e.target.value))}
                          />
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          type="date"
                          className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-blue-600 font-medium"
                          value={selectedCar.TERAKHIR_SERVICE ? selectedCar.TERAKHIR_SERVICE.split("T")[0] : ""}
                          onChange={(e) =>
                            setSelectedCar({
                              ...selectedCar,
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center">
                              ...
                              <span className="font-bold ml-2">{selectedCar.TOP_SPEED} km/h</span>
                            </div>
                            <div className="flex items-center">
                              <i className="fas fa-money-bill-wave mr-3 text-gray-600 w-5"></i>
                              <span className="text-gray-700 font-medium">Harga Sewa:</span>
                              <span className="font-bold ml-2 text-green-600">{formatCurrency(selectedCar.HARGA)}</span>
                            </div>
                            <div className="flex items-center">
                              <i className="fas fa-calendar-alt mr-3 text-gray-600 w-5"></i>
                              <span className="text-gray-700 font-medium">Tahun Beli:</span>
                              <span className="font-bold ml-2">{selectedCar.TAHUN_BELI}</span>
                            </div>
                            <div className="flex items-center">
                              <i className="fas fa-chart-line mr-3 text-gray-600 w-5"></i>
                              <span className="text-gray-700 font-medium">Jumlah Sewa:</span>
                              <span className="font-bold ml-2">{selectedCar.BANYAK_SEWA} kali</span>
                            </div>
                            <div className="flex items-center">
                              <i className="fas fa-wrench mr-3 text-gray-600 w-5"></i>
                              <span className="text-gray-700 font-medium">Terakhir Service:</span>
                              <span className="font-bold ml-2">{formatDate(selectedCar.TERAKHIR_SERVICE)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="pt-4 border-t border-gray-200">
                        ...
                        >
                          <i
                            className={`fas ${selectedCar.STATUS === "Tersedia" ? "fa-check-circle" : "fa-times-circle"} mr-2`}
                          ></i>
